    ...
    def motor_callback(self, motor_name, msg_type, data):
        ...
        if msg_type == "steps_cursor":
            if motor_name in ["left_wheel", "right_wheel"]:
                ...
                left_distance  = self.pulse_odometer["left_wheel"]  / self.steps_per_rotation * self.wheel_circumference * ( 1 if settings.motor_control["direction"] else -1)
                right_distance = self.pulse_odometer["right_wheel"] / self.steps_per_rotation * self.wheel_circumference * ( 1 if settings.motor_control["direction"] else -1)
